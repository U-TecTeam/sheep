  ...
  return (
    <div className="min-h-screen bg-[#FBFBFB] font-['Space_Grotesk'] text-black pb-40">
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-gray-100 px-6 py-4 flex items-center justify-between">
        <h1 className="text-xl font-bold uppercase tracking-widest">{tNav('shopping_cart')}</h1>
        <button onClick={() => router.push('/')} className="text-sm font-bold text-gray-400">{tNav('close')}</button>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-6 pt-10 space-y-10">
        ...
        <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-6">
          <div className="flex justify-between items-end">
            <div className="space-y-1">
              <h2 className="text-lg font-bold">{t('free_shipping_threshold', { threshold: shippingThreshold })}</h2>
              <p className="text-sm text-gray-400 font-medium">
                {remainingForFreeShipping > 0 
                  ? t('remaining_for_free', { amount: remainingForFreeShipping }) 
                  : t('satisfied_free')}
              </p>
            </div>
        ...
        <section className="space-y-4">
          {items.length === 0 ? (
            <div className="text-center py-20 space-y-4 bg-white rounded-[2.5rem] border-2 border-dashed border-gray-200">
              ...
              <button onClick={() => router.push('/')} className="text-sm font-bold underline underline-offset-4">{t('go_shopping')}</button>
            </div>
          ) : (
            items.map((item) => (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-1 uppercase tracking-widest font-bold">
                    {item.purchaseType === 'subscription' 
                      ? t('subscription', { frequency: item.frequency === '1month' ? t('monthly') : t('biweekly') }) 
                      : t('once')}
                  </p>
        ...
        <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-6">
          <div className="flex items-center gap-2">
            <MapPin size={18} />
            <h2 className="text-sm font-bold uppercase tracking-widest">{t('address_parsing')}</h2>
          </div>
          ...
          {parsedAddress && (
            ...
            >
              <div className="space-y-1">
                <p className="text-[10px] text-gray-400 uppercase font-bold tracking-widest">{t('recipient')}</p>
                <p className="font-bold">{parsedAddress.name}</p>
              </div>
              <div className="space-y-1">
                <p className="text-[10px] text-gray-400 uppercase font-bold tracking-widest">{t('phone')}</p>
                <p className="font-bold">{parsedAddress.phone}</p>
              </div>
              <div className="col-span-2 space-y-1 border-t border-gray-200 pt-3">
                <p className="text-[10px] text-gray-400 uppercase font-bold tracking-widest">{t('address')}</p>
          ...
          <section className="bg-black text-white p-8 rounded-[2.5rem] space-y-8 shadow-xl">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold uppercase tracking-widest text-gray-400">{t('order_tracking')}</h2>
              </div>
              <span className="text-[10px] font-bold bg-white text-black px-2 py-1 rounded">{t('shipping')}</span>
            </div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-6 z-50 shadow-[0_-20px_40px_-20px_rgba(0,0,0,0.1)]">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-[10px] text-gray-400 uppercase font-bold tracking-widest">{t('total_amount')}</p>
